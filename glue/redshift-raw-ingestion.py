...
# Define constants
bucket_name = "s3://files-csv-apr2026-lab-1"
redshift_iam_arn = "arn:aws:iam::248949156360:role/service-role/AmazonRedshift-CommandsAccessRole-20260416T044835"
...
# Initialize conn to None so finally block is always safe
conn = None
...
try:
    ...
    if table_name == 'apartment_viewings':
        merge_cmd = """
            ...
            ON raw_zone.apartment_viewings.apartment_id = source.apartment_id
            AND raw_zone.apartment_viewings.user_id = source.user_id
    ...
    logger.info(f"Temporary table tmp_{table_name} truncated.")
    ...
    if conn is not None:
        ...
        logger.warning("Connection was never established, nothing to close.")